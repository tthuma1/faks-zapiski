...
- TIMx_CCMR1 = capture compare mode register:
	...
	- CC1S = selection:
		...
		- 00 = izhodni (output) način; kanal upravlja z GPIO pinom
	...
	- TIMx_CCER = capture compare enable register:
		...
		- polariteta = kakšna naj bo vrednost na izhodnem pinu, ko je OC enak CNT  (ali naj bo na izhodu 0 ali 1) - ko je OC enak CNT, temu rečemo aktivno stanje; če je aktivno stanje naše naprave 0, potem rabimo polariteto 1; kakšno bo stanje pina na začetku (na začetku bo pin neaktiven)
		- kaj pomeni, da je pin aktiven, je odvisno od dogovora - lahko je aktiven v visokem (pozitivna polariteta) ali v nizkem stanju (negativna polariteta) 
...
void Init_TIM3_OC1() {
	...
	TIM3->CCMR1 = TIM3->CCMR1 & 0xfffffffc; // CC1A = 00 (kanal OC1 bo krmilil PIN (output), deluje v output načinu)
	...
	TIM3->DIER = TIM3->DIER | 0x02; // omogoči prekinitev iz CC1 kanala
...
- OC povečamo naprej vedno, ko se proži prekinitev (prekinitev se proži, ko CNT == OC)
...
- v `startup_stm32h753xx.s` imamo že napisane ISR, med njimi tudi TIM3_IRQHandler kot `weak` pointer
- ISR mora ugotoviti, zakaj je prišlo do prekinitve in pobrisati prekinitvene zastavice:
	- `HAL_TIM_IRQHandler(&htim3);` - ugotovi, ali je prekinitev prišla iz CC kanala (kaj je razlog za prekinitev iz časovnika)
	- krivec za prekinitev je lahko update event, kanal 1, kanal 2, napake ...
	- v `HAL_TIM_IRQHandler` se kliče funkcija `HAL_TIM_OC_DelayElapsedCallback`, ki je definirana kot `weak` - zato jo prepišemo in v njej napišemo `if (htim->Instance ==  TIM3) TIM3->CCR1 += 300;`

```c
int main() {
	// nastavi prekinitve na timerju
	Set_NVIC_SetPriority(TIM3_IRQn, ....);
	Set_NVIC_Enable(TIM3_IRQn);
}
```


- če onemogočimo prekinitve, se nam bo toggle dogajal na periodo, ki jo določa ARR