...
- ustvarjalni, strukturni, vedenjski vzorci
- posledice uporabe vzorcev:
	- dodatni razredi, povezave, klici
	- večja modularnost
	- koristno je samo, kadar je dodana fleksibilno vredna truda

- vzorci imajo ime, opis področja, opis rešitve (lahko z diagrami), posledice uporabe
- tvorjenje objektov na alternativne načine
- včasih ne vemo vnaprej, kakšen objekt moramo ustvariti
- naj se objekti naredijo iz podrazredov
- strukturni vzorci
- vedenjski vzorci:
	- načini, kako objekti komunicirajo med seboj

Abstract factory:
- factory (tovarna) proizvaja objekte na določen način
- omogoča, da naredimo objekte s skupno tematiko na enak način - predpišemo njihvoo tvorbo v podrazredu
- builder - loči konstrukcijo od predstavitve

Singleton:
- samo ena instanca classa za vse možne uporabe

Adapter:
- iz enega vmesnika v drugega

Composite:
- drevesna struktura

Decorator:
- v našemu razredu kasneje določimo neke lastnosti

- ![500](../../Images4/Pasted%20image%2020251211132749.png)

Factory:
- imamo eno glavno tovarno in podrazrede (podtovarne) - vsaka podtovarna na drugačen način naredi produkt

Dekorator:
- namen je dinamično dodajanje lastnosti objektu
- dodatek vsebuje referenco na objekt, ki mu dodaja lastnosti
- ni nujno, da na začetku že vemo, da bo dodatek (decorator) obstajal

Iterator:
- moramo omogočiti način, da pregledamo vse elemente na enk način
- razred lahko naredi iterator - iterator ima operacije first(), next() ...

Abstraktna tovarna:
- govorimo o razredih, ne o objektih
- naredimo razred, ki predstavlja vmesnik za implementacijo objektov, brez da bi povedali konkreten razred
- vrne abstrakten kazalec na razred, zares pa se naredi konkreten objekt, ki redefinira abstraktne metode

Singleton:
- včasih rabimo nek objekt določene vrste, ampak ga ne naredimo z namenom, da bomo naredili veliko objektov te vrste
- ob klicu metode želimo vedno dobiti isti objekt, ne ustvarjati novih
- namesto kontruktorja imamo "pomožni konstruktor", ki vedno vrača isti objekt

Antivzorci:
- so načrtovalski vzorci, ki kažejo negativne, neučinkovite rešitve
- da se zavedamo, kako se ne dela
- pogoste napake v kodi - zakaj je rešitev privlačna in kako bi bilo boljše
- opišemo od kod se pojavijo te slabe rešitve in kako bi stvar popravili