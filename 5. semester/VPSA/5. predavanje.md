...
Pisatelji + bralci:
- imamo več pisateljev, ki lahko pišejo v knjigo in veliko bralcev, ki hočejo takoj brati kar se napiše
- pravila:
	- v knjigo lahko piše samo en pisatelj na enkrat
	- lahko je hkrati več bralcev
...
- knjiga je omejena dobrina - imamo ključavnico za knjigo:
...
```go
		...
		lockReaders.Unlock()
```
- ^ pomembno je, da je `lockReaders.Unlock()` na koncu (da je `if` pred `lcokReaders.Unlock()`), ker bi se sicer lahko večkrat `activeReaders` povečal, preden bi ga primerjali z 1
...
- bralci so trenutno zelo nasilni:
	- ko bralci dobijo knjigo, berejo dalj časa - veliko več bo branja kot pisanja
	- razlog: imamo več bralcev kot pisateljev
	- bralci stradajo pisatelje
	- želimo si, da imajo pisatelji prednost