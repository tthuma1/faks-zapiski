...
- pri Apptainer po default vidimo del matičnega OS znotraj vsebnika (npr. naš home folder vidimo - pri Docker se to ne zgodi)

Kako narediti vsebnik:
- najlažje je vzeti obstoječi vsebnik in ga nadgraditi
...
Apptainer uporaba:
- `apptainer run file.sif`
- `apptainer exec file.sif`
- `apptainer shell file.sif`
- `apptainer build modrec.sif docker://ubuntu`
- .sif so read-only, če želimo spreminjati, rabimo `apptainer build --sandbox modrec.sb docker://ubuntu` in `apptainer shell --writable --fakeroot ./modrec.db`
...
```
- post se zažene stvari ob build time, environemnt pa ob run time
- lahko naredimo, da vsebnik pobere stvari iz naše mape - jo moramo mountati v vsebnik:
	- `apptainer run --bind ./modrosti/:/usr/share/games/fortunes modrec.sif ./pregovori/` - smo si zakrili staro vsebino mape `/usr/share/games/fortunes`, ampak vsaj nismo rabili nove slike buildati

- fino je, da vsebnik dokumentiramo s %help, napišemo teste s %test
- če rabimo dati neke datoteke v vsebnik, jih lahko skopiramo iz lokalnega datotečnega sistema - to je smotano, ker rabimo zraven vsebnika shippati še te datoteke:
	- bolje je, če jih gre vsebnik fetchati nekam na internet ob build time