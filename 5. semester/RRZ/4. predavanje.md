...
- jedro je ločljivo (seperabilno), če ga lahko izrazimo kot konvolucijo večih jeder
- povprečen in Gaussov filter je seperabilen
- če povprečni filter ločimo na dve jedri 1x3 in 3x1, bomo dobili 6 množenj na piksel, namesto 9
- niso vsa jedra ločljiva
- Gaussov filter razbijemo na filter po x in filter po y:
	- ![500](../../Images4/Pasted%20image%2020251028172919.png)

- če bi imel filter padding z ničlami, bi bilo isto, kot če teh ničen ne bi bilo

Pomankljivost linearnih filtrom:
- "zapackamo" okolico piksla, če smo na robu ali če imamo impulzni šum
- ![600](../../Images4/Pasted%20image%2020251028173445.png)
- zgladi robove, črte

Nelinearni filtri:
- spet imamo koeficiente v jedru, ampak funkcija ni linearna (ni množenje in seštevanje)
- min, max, mediana
- minimalni filter:
	- odstrani špice (sol pri salt and pepper šumu)
	- vse impulze, ki so ožji od širine filtra, bo dal na 0
- minimalni filter odstrani bele pike in razširi temne regije
- maksimalni filter odstrani črne pike in razširi svetle regije
- minimalni filter odstrani sol, maksimalni pa poper - kako bi oboje odstranili:
	- uporabimo medianin filter, da bodo nenadni impulzi v obe smeri izginili

Medianin filter:
- izolirane točke, tanke črte se odstranijo
- sredinski piksel je najbolj verjetno pravi, zato ga želimo podobno kot pri Gaussovem filtru poudariti:
	- to naredimo tako, da damo sredinski piksel večkrat noter v tabelo, ki jo bomo sortirali
	- ![500](../../Images4/Pasted%20image%2020251028175111.png)
	- ^sredinski piksel damo trikrat v tabelo, zgoraj/spodaj/levo/desno damo dvakrat, kote damo enkrat v tabelo
	- lahko spreminjamo koeficiente, podobno kot pri linearnih filtrih

Robovi slike:
- ponavadi ni panike, če začnemo računati malo bolj na sredini, ker tisto, kar gledamo, ni čisto na robu
- da nimamo problema računanja na robovih, sliko razširimo s paddingom:
	- lahko damo vrednosti 0, srednjo vrednost, zadnji piksel raztegnemo, zrcalimo, naredimo ciklično ...
	- ponavadi je vseeno, ker kar je na robovu običajno ni pomembno, zato daš samo srednjo vrednost